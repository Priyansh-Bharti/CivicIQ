...
export const Landing = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {/* Hero Section */}
      <section className="bg-navy min-h-[90vh] flex items-center pt-16">
          ...
          >
            ...
            >
              <button 
                ...
                className="bg-amber text-navy px-8 py-4 rounded-md font-bold text-lg hover:scale-105 transition-transform"
              >
                Explore the process
              </button>
              <button 
                onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })}
                className="border border-white text-white px-8 py-4 rounded-md font-bold text-lg hover:bg-white/10 transition-colors"
              >
                How it works
              </button>
            </motion.div>
      ...
      <section className="py-20 bg-white border-y border-gray-100 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h3 className="text-center text-sm font-bold text-indigo uppercase tracking-[0.2em] mb-10">Your election journey</h3>
              ...
              {ELECTION_PHASES.map((phase, idx) => (
                <div key={phase.id} className="flex items-center gap-4 shrink-0">
                  <button
                    ...
                    className="bg-indigo text-white px-6 py-3 rounded-full font-medium shadow-md hover:bg-indigo/90 transition-colors whitespace-nowrap"
                  >
                    {phase.name}
                  </button>
      ...
      <section className="py-12 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
            ...
            <div className="flex items-center gap-2 font-bold text-gray-600">
              <Cloud className="w-5 h-5" /> Firebase
            </div>
            <div className="flex items-center gap-2 font-bold text-gray-600">
              <Shield className="w-5 h-5" /> Google Cloud Run
            </div>
            <div className="flex items-center gap-2 font-bold text-gray-600">
              <Shield className="w-5 h-5" /> WCAG 2.1 AA
            </div>
          </div>
        </div>
      </section>
